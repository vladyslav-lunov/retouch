...
class DetectParams:
    ...
    Число НЕ переноситься між кадрами: на 44 реальних кадрах той самий
    поріг дав від 0% до 24% торкнутої шкіри (spec.md §6.2). Якщо кадри
    різні — задавай target_coverage і не чіпай це поле."""

    target_coverage: float | None = None
    """Замість порога — ЦІЛЬ: яку частку шкіри дозволено торкнутися.
    Поріг тоді підбирається під кадр, а це поле лишається тим самим для
    всієї зйомки. Саме воно переноситься між кадрами, а не контраст:
    заміряно, що з фіксованим порогом 0.012 із 44 кадрів у робочу зону
    потрапляє 15, а 14 переходять у згладжування (spec.md §6.2).
    0.03 — розумний старт: прибрати помітне, не чіпаючи текстуру."""
