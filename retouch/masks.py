...
class MaskParams:
    ...
    max_faces: int = 4
    """Скільки облич розбирати на кадрі. Родинний кадр з одним розібраним
    обличчям означає, що решті людей ретуш просто не дісталась — а на
    реальній зйомці таких кадрів половина (spec.md §15.3). Чотири, а не
    «всі»: кожне обличчя коштує окремого проходу моделі (~1.5 с на 26 Мп),
    і на груповому знімку з десятьма людьми ретуш кожного облич­чя
    все одно не має сенсу — вони завузькі."""

    min_face: int = 120
    """Найвужче обличчя в px, яке ще беремо. Дрібніше модель розбирає
    навмання, і це додає шум у маску, а не ще одну людину."""
...
def face_crop_box(img: np.ndarray, box, margin: float = 0.7) -> tuple[int, int, int, int]:
    """КВАДРАТНА рамка кропа навколо обличчя.

    Квадратна — і це головне. BiSeNet бере вхід 512x512, тож будь-який
    кроп розтягується до квадрата. Попередня рамка була портретною
    (сторони 1506x2194, тобто 0.69), обличчя в ній стискалось по
    горизонталі на третину, і модель починала вигадувати: на родинному
    кадрі вона позначала «шкірою» сукню в горошок, і лікування чесно
    ретушувало горошини.

    Заміряно на чотирьох реальних кадрах: частка маски ПОЗА рамкою
    обличчя падає з 61.5% до 29.7% і з 49.7% до 36.8% там, де в кадрі є
    торс, і не змінюється (0.5%) на щільних портретах. Частка шкіри
    ВСЕРЕДИНІ рамки при цьому та сама ±2 пп — тобто обличчя не втрачаємо,
    втрачаємо тільки вигадане.

    Розмір веде більша зі сторін обличчя, бо рамка YuNet вужча за голову:
    волосся й підборіддя мають лишитись у кропі.

    Запас 0.7 підібрано на чотирьох групових кадрах: він найкращий або
    нарівні на всіх, а на щільних портретах не міняє нічого (частка маски
    поза рамками там 0-1% при будь-якому запасі). Проти попередніх 0.85
    дотики до сукні на IMG_0798 падають з 16616 до 3753 пікселів.

    Розрізняти тут варто дві речі. КВАДРАТ — рішення принципове: вхід
    моделі квадратний, і розтягнуте обличчя вона розбирає як завгодно.
    ЗАПАС — емпіричний, зважений на чотирьох кадрах, і його не гріх
    переміряти на більшій вибірці.
    """
    h, w = img.shape[:2]
    x, y, fw, fh = box
    r = int(max(fw, fh) * margin)
    cx, cy = x + fw // 2, y + fh // 2
    # Спершу СУНЕМО квадрат у межі кадру і тільки потім ріжемо: обличчя
    # біля краю інакше дістало б прямокутник, тобто те саме спотворення.
    x0 = min(max(0, cx - r), max(0, w - 2 * r))
    y0 = min(max(0, cy - r), max(0, h - 2 * r))
    return x0, y0, min(w, x0 + 2 * r), min(h, y0 + 2 * r)
...
class FaceParser:
    ...
    def parse(self, img: np.ndarray, detector: str | Path | None = None,
              margin: float | None = None, max_faces: int = 1,
              min_face: int = 120) -> np.ndarray:
        ...
        `max_faces` — скільки облич розбирати. Родинний кадр з одним
        розібраним обличчям означає, що решті людей ретуш просто не
        дісталась, а на зйомці це половина кадрів (spec.md §15.3).

        Кропи сусідніх облич перетинаються, тож пишемо кожне ЛИШЕ туди,
        де ще фон. Обличчя відсортовані за спаданням, отже в перетині
        виграє більше — те, чий кроп центрований на ньому самому, а
        значить чий розбір надійніший.

        `min_face` — нижня межа в пікселях. Обличчя, вужче за неї, модель
        розбирає навмання, і додати його означає додати шум у маску, а не
        ще одну людину.
        """
        self.last_faces = []
        if not detector:
        ...
        if not faces:
            # Провал детектора — це НЕ дрібниця, а падіння на шлях, який
            # §5 називає непридатним: на повному кадрі 26 Мп BiSeNet дає
            # 1.4% шкіри і 28% «капелюха». Мовчки продовжити, лишивши в
            # звіті «+yunet», означає приховати саме те, що зламалось.
            print("[masks] УВАГА: YuNet не знайшов обличчя — розбираю ПОВНИЙ "
                  "кадр, а це ненадійно (spec.md §5). Ширина обличчя тоді "
                  "береться з габариту маски й може бути хибною (§6.3)",
                  flush=True)
            return self._parse_whole(img)
        h, w = img.shape[:2]
        out = np.zeros((h, w), np.int32)
        used = []
        for box in faces[:max(1, int(max_faces))]:
            if box[2] < min_face and used:
                break                    # відсортовано за спаданням — далі дрібніші
            # Запас лишається в face_crop_box: дефолт у двох місцях уже
            # розійшовся один раз — parse передавав 0.85 після того, як
            # рамка стала квадратною з 1.15, і модель бачила менший кроп,
            # ніж думав автор.
            x0, y0, x1, y1 = (face_crop_box(img, box) if margin is None
                              else face_crop_box(img, box, margin))
            part = self._parse_whole(img[y0:y1, x0:x1])
            view = out[y0:y1, x0:x1]
            free = view == 0             # чуже обличчя вже розібране — не чіпаємо
            view[free] = part[free]
            used.append(box)
        faces = used
